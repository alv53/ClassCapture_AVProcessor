/*
 * Credit: Nghia Ho, http://nghiaho.com/?p=2093
 */
...
int stabilize(char* filename, int rotation)
{
    ...
    cout << filename << "filename" << cap.get(CV_CAP_PROP_FPS) << ',' << Size(cap.get(CV_CAP_PROP_FRAME_WIDTH), cap.get(CV_CAP_PROP_FRAME_HEIGHT)) << endl;
    // VideoWriter out("out.mp4", 
    //            CV_FOURCC('X', 'V', 'I', 'D'), 
    ...
    while(k < max_frames-1) { // don't process the very last frame, no valid transform
        ...
        Mat canvas = Mat::zeros(cur2.rows, cur2.cols, cur2.type());
        cur2.copyTo(canvas(Range::all(), Range::all()));
    
        if (rotation == 90) {
            transpose(canvas, canvas);
